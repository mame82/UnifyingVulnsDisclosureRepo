...
func NewKeyboard(ctx context.Context, resourcePath string) (keyboard *HIDKeyboard, err error) {
...
func (kbd *HIDKeyboard) LoadLanguageMapDir(dirpath string) (err error) {
	...
	err = filepath.Walk(string(folder), func(path string, info os.FileInfo, err error) error {
		if err != nil {
			return err
		} // prevent panic due to access failures
...
func (kbd *HIDKeyboard) StringToKeyCombo(comboStr string) (result []KeyboardOutReport, err error) {
	...
	if len(keyNames) == 0 {
		result = []KeyboardOutReport{ KeyboardReportEmpty}
		return
	...
	//combine reports
	pComboReport, eCombine := combineReports(comboReports)
	if eCombine != nil {
		err = eCombine
		return
	}
	result = []KeyboardOutReport{*pComboReport, KeyboardReportEmpty}
	return
}


func (kbd *HIDKeyboard) StringToPressKeySequence(str string) (resultReports []KeyboardOutReport, err error) {

	//ToDo: Check if keyboard device file exists
	if kbd.ActiveLanguageLayout == nil {
		err = errors.New("No language mapping active, couldn't send key sequence!")
		return
	}

	resultReports = make([]KeyboardOutReport,0)
	for _, runeVal := range str {
		...
		if reports, found := kbd.ActiveLanguageLayout.Mapping[strRune]; found {
			...
			*/
			resultReports = append(resultReports, reports...)
			empty := KeyboardReportEmpty
			resultReports = append(resultReports, empty) // add key release
		} else {
...
func (kbd *HIDKeyboard) mapKeyNameToReport(keyName string) (report *KeyboardOutReport, err error) {
	...
	if rpSingleUpperLetter.MatchString(keyName) {
...
func (kbd *HIDKeyboard) PressKeySequence(reports []KeyboardOutReport) (err error) {
	...
	delay := kbd.KeyDelay
	if kbd.KeyDelayJitter > 0 {
		delay += rand.Intn(kbd.KeyDelayJitter)
	}
	if delay > 0 {
...
func (kr *KeyboardOutReport) MarshalJSON() ([]byte, error) {
	...
	if kr.Modifiers&HID_MOD_KEY_LEFT_CONTROL > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_LEFT_CONTROL).String())
	}
	if kr.Modifiers&HID_MOD_KEY_LEFT_SHIFT > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_LEFT_SHIFT).String())
	}
	if kr.Modifiers&HID_MOD_KEY_LEFT_ALT > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_LEFT_ALT).String())
	}
	if kr.Modifiers&HID_MOD_KEY_LEFT_GUI > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_LEFT_GUI).String())
	}
	if kr.Modifiers&HID_MOD_KEY_RIGHT_CONTROL > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_RIGHT_CONTROL).String())
	}
	if kr.Modifiers&HID_MOD_KEY_RIGHT_SHIFT > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_RIGHT_SHIFT).String())
	}
	if kr.Modifiers&HID_MOD_KEY_RIGHT_ALT > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_RIGHT_ALT).String())
	}
	if kr.Modifiers&HID_MOD_KEY_RIGHT_GUI > 0 {
		modifiers = append(modifiers, HIDMod(HID_MOD_KEY_RIGHT_GUI).String())
	}

	for _, key := range kr.Keys {
		if key == 0 {
			break
		} //abort on first 0x00 key code
...
func NewKeyboardOutReport(modifiers HIDMod, keys ...HIDKey) (res KeyboardOutReport) {
	res = KeyboardOutReport{
		Keys: [6]HIDKey{0, 0, 0, 0, 0, 0,},
	}
